...
import { t as createSsrRpc } from "./createSsrRpc-8XVV0IZ4.mjs";
...
import { n as Separator, t as Checkbox } from "./separator-bytZIaPI.mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/routes-C5esbCKf.js
...
function useLookbook() {
	...
	return {
		...
		saveLook: (0, import_react.useCallback)((look) => {
			...
			if (client) client.auth.getUser().then(({ data }) => {
				if (!data.user) return;
				client.from("lookbook_entries").insert({
					id: entry.id,
					user_id: data.user.id,
